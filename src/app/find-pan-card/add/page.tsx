...
const schema = z.object({
  ...
  aadhaarAttachment: z.any().optional(),
  remarks: z.string().optional(),
  findPanCharge: z.coerce.number().min(0, "Find PAN charge is required"),
  totalCharge: z.coerce.number().min(0, "Total charge is required"),
});

type FindPanFormData = z.infer<typeof schema>;

function Page() {
  const form = useForm<FindPanFormData>({
    resolver: zodResolver(schema),
    defaultValues: {
      personName: "",
      mobileNumber: "",
      aadhaarAttachment: undefined,
      remarks: "",
      findPanCharge: 30,
      totalCharge: 30,
    },
  ...
  const findPanCharge = useWatch({
    control: form.control,
    name: "findPanCharge",
  });

  React.useEffect(() => {
    form.setValue("totalCharge", Number(findPanCharge || 0), {
      shouldValidate: true,
    });
  }, [findPanCharge, form]);

  const onSubmit = (data: FindPanFormData) => {
    console.log("Find PAN submitted", data);
  };
            ...
            <form className="space-y-8" onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <FormField
                ...
                render={({ field }) => {
                  ...
                  return (
                    <FormItem>
                      <FormLabel>Attach Aadhaar Front and Back</FormLabel>
                      <FormControl>
                        <Input
                          type="file"
                          {...rest}
